  ...
  <div>
    <div class="mainTable m-2">
      ...
      <div class="mx-3 my-2 d-flex align-items-center justify-content-between">
        ...
        <span
          class="newContact d-flex align-items-center justify-content-center"
...
import {
  ...
  updateContactStage,
} from "../services/ApiPrivateService";
...
export default {
  ...
  methods: {
    ...
    async updateStage(evt) {
      try {
        if (evt.added) {
          ...
          await updateContactPosition(contact.id, {
    ...
    updateFilteredContacts() {
      this.filteredContactsList = this.contacts.filter(
        (contact) => contact.stage_id === this.stageId
      );
    },
  },
  created() {
